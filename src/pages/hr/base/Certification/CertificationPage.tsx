  ...
  return (
    <Page style={{ display: 'flex' }} title="증명서 발급">
            ...
            <FormControl fullWidth style={{ width: '100%', marginBottom: '10px', fontSize: '14px' }}>
              ...
              >
                {certificateList.map((certificate, index) => (
                  <MenuItem value={certificate} key={index}>
                    {certificate}
                  </MenuItem>
                ))}
              </Select>
      ...
      <div ref={componentRef} style={{ width: '100%', height: window.innerHeight, overflow: 'visible', flex: 2 }}>
        {visible && certificate === certificateList[0] && <CertificateOfEmployment deptName={deptName} empName={empName} />}
        {visible && certificate === certificateList[2] && <CertificateOfRetirement deptName={deptName} empName={empName} />}
      </div>
    </Page>
  );
}
